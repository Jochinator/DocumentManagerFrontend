...
export class DocumentDetailsComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private client: HttpClient, private sanitizer: DomSanitizer) { }
  ...
  ngOnInit(): void {
    this.route.params.subscribe((value: any) => this.client.get<DocumentMetadata>("api/Document/" + value.id).subscribe(value1 => {
      ...
      this.sanitizedDocumentLink = this.sanitizer.bypassSecurityTrustResourceUrl("api/"+this.metadata.filePath);
    }))
  }

  save() {
    if (this.metadata)
      this.client.put("api/Document/" + this.metadata.id, this.metadata).subscribe();
  }
}
